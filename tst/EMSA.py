...
class EMSA(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        #
        # print(self.up(identity).shape)
        # print(self.up(identity).transpose(-2, -3).flatten(2).transpose(-1, -2).shape)
...
if __name__ == '__main__':
    input = torch.randn(128, 200, 512)
    #input = torch.randn(128, 200, 512)
    emsa = EMSA(d_model=512, d_k=512, d_v=512, h=8, H=10, W=20, ratio=2, apply_transform=True)
