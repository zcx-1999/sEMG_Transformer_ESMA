...
import metric
from base.RNN_Model import RNN
...
def get_args():
    ...
    # model
    parser.add_argument('--model_name', default='LSTM') #AdaRNN
...
def train(obj,args):
    ...
    loss_fun = nn.L1Loss()
    writer = tb.SummaryWriter(log_dir='./newlog/LSTM/'+args.model_name+'miu_0111')
    ...
    model = RNN(input_size=12,output_size=10,rnn_model=args.model_name,hidden_size=128,num_layers=3,dropout=0.3).cuda()
    print(model)
    ...
    for i in range(400):
        ...
        with torch.no_grad():
            ...
            pprint("Epoch[{:d}|350] Train Loss = {:.4f} Test Loss = {:.4f}".format(i, loss_epoch / lss,loss_epoc/len(test_data)))
        ...
        if (max_test < aver_test):
            ...
            torch.save(model, './save/new/LSTM/cross'+ args.model_name + "miu_sub" + str(obj) + '_' + str(args.batch_size) + '_3_0.3' + '.pkl')
